...
//    List<MoimReply> findMoimRepliesByMoimIdDesc(@Param("moimId") Long moimId);
 ...
 // moim_comm_no로 MoimReply를 찾아서 rno 내림차순으로 정렬하는 메서드
    @Query("SELECT mr FROM MoimReply mr WHERE mr.moimCommNo = :moimCommNo ORDER BY mr.rno DESC")
    List<MoimReply> findMoimRepliesByMoimCommNoDesc(@Param("moimCommNo") Long moimCommNo);
